...
<div class="absolute left-1/2 z-100 mt-2 flex w-full -translate-x-1/2" ref="searchResults">
    <div class="flex-auto overflow-hidden rounded-md bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
        ...
        <div class="p-4 w-full">
            ...
            <li v-for="category in result.categories" :key="category.id" class="group relative gap-6 rounded-lg p-4 hover:bg-gray-50">
...
const { data:result, pending, execute } = await useApiFetch(() => `search/${props.query}`, {immediate:false})
